...
class AIService {
  ...
  async enhanceContent(content, fieldType, context) {
    ...
    if (!this.hasOpenAI) {
      return {
        content: this.generateFallbackContent(content, fieldType, context),
        tokensUsed: 0,
        isAIGenerated: false
      };
    }
...
Enhanced content:`;
  ...
  generateFallbackContent(content, fieldType, context) {
    ...
    const fallbackTemplates = {
      'summary': `${career || 'This career'} is a dynamic and evolving field that offers numerous opportunities for growth and innovation. Professionals in this domain work with cutting-edge technologies and methodologies to drive meaningful impact in their organizations.
...
With the increasing demand for skilled professionals in this area, ${career || 'this career'} represents a stable and rewarding career choice that continues to evolve with technological advancements and industry needs.`,

      'title': `${content || career || 'Career'} - Complete Career Guide & Opportunities`,
...
• Industry outlook: Growing demand with technological advancement`,
    ...
    return `Comprehensive information about ${career || 'this'} career opportunities, requirements, and growth prospects. This field offers excellent potential for professional development and meaningful work in today's evolving job market.`;
  }
  ...
  async generateCareerImage(careerTitle, options = {}) {
    const prompt = this.generateCareerImagePrompt(careerTitle);
    return await this.generateImage(prompt, options);
  }

  generateCareerImagePrompt(careerTitle) {
    return `Professional illustration of ${careerTitle} career, modern workplace setting, diverse professionals working, clean and inspiring, corporate style, high quality, realistic`;
  }

  generateFallbackImageUrl(careerTitle) {
    // Return a placeholder image service URL
    const encodedTitle = encodeURIComponent(careerTitle);
    return `https://via.placeholder.com/1200x600/4F46E5/FFFFFF?text=${encodedTitle}+Career`;
  }
}
